...
const InvoicePreview = () => {
  ...
  if (!data) {
    return (
      <div className="p-10 text-theme-primary">Loading invoice data...</div>
    );
  }

  const localFirm = JSON.parse(localStorage.getItem("selectedFirm") || "{}");
  const firm =
    data.firmId && typeof data.firmId === "object"
      ? data.firmId
      : data.firm || localFirm;
  ...
  const handlePrint = () => {
    window.print();
    const targetPath =
      location.state?.from === "history" ? "/history" : "/invoice-form";
    const targetLabel =
      location.state?.from === "history" ? "History" : "Builder";
    const targetState =
      location.state?.from === "history" ? location.state?.originalState : null;
  ...
  const uploadToCloudinary = async () => {
    ...
    try {
      ...
      console.log(
        "Preparing to fetch:",
        `${import.meta.env.VITE_API_BASE_URL}/api/invoices/${data._id}/upload`,
      );
    ...
    <div className="page-container no-print-bg">
      <div className="content-max-width mb-6 flex flex-col md:flex-row md:items-center justify-between gap-6 no-print">
        ...
        >
          ...
          {location.state?.from === "history" ? "History" : "Editor"}
        </button>
              ...
              {uploadStatus === "error" && (
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400">
                    ...
                    <span className="text-xs font-bold uppercase tracking-wider text-nowrap">
                      Upload Fail
                    </span>
                  </div>
                  <button
      ...
      >
        ...
        <div className="flex justify-between items-start mb-4 border-b-2 border-black pb-4">
          <div className="w-32 h-32 flex items-center justify-center">
            <div className="bg-[#555] p-2 relative">
              <div className="border-2 border-white/50 p-2">
                <span className="text-white text-7xl font-serif italic font-black">
                  A
                </span>
              </div>
          ...
          <div className="flex-1 text-center flex flex-col items-center">
            <h1
              className="text-4xl font-black text-gray-800 tracking-tighter mb-2 uppercase"
              style={{ fontFamily: "sans-serif" }}
            >
              {firm.name || "AARAMBH JEWELS"}
            </h1>
            <p className="text-xl font-bold tracking-[0.2em] text-gray-700 mb-1">
              GOLD AND SILVER
            </p>
            <p className="text-sm font-bold text-gray-600 uppercase">
              {firm.address || "STORE KESLA"}
            </p>
            <p className="text-sm font-bold text-gray-600 uppercase">
              Phone : {firm.phone || "8827375018"}
            </p>
          </div>
          ...
          <div className="w-32 h-32 flex items-center justify-center">
            ...
            >
              ...
              <text
                ...
                textAnchor="middle"
                fontSize="6"
                className="fill-black stroke-none font-bold"
              >
                मानक: पथप्रदर्शक:
              </text>
            </svg>
        ...
        <div className="flex justify-between mb-4 text-[11px] leading-tight">
          <div className="space-y-1">
            ...
            <p>
              <span className="w-16 inline-block font-bold">NAME</span> :{" "}
              {customerName.toUpperCase()}
            </p>
            <p>
              <span className="w-16 inline-block font-bold">ADDRESS</span> :{" "}
              {customerAddress.toUpperCase()}
            </p>
            <p>
              <span className="w-16 inline-block font-bold">PHONE</span> :{" "}
              {customerPhone}
            </p>
          </div>
          <div className="text-right space-y-1">
            ...
            <p>
              <span className="font-bold">DATE:</span> {date.toUpperCase()}
            </p>
            <p>
              <span className="font-bold">GSTIN :</span>{" "}
              {firm.gstNumber || "23EXDPS7162D1ZG"}
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-[10px] text-center border-collapse">
            <thead>
              <tr className="bg-gray-100 border-b-2 border-black font-bold">
                ...
                <th className="border-r-2 border-black p-1">
                  HALLMARK CHARGES
                </th>
            ...
            <tbody>
              {items.map((item, idx) => (
                <tr
                  key={idx}
                  className="border-b-2 border-black h-12 align-middle"
                >
                  <td className="border-r-2 border-black px-1 text-left font-bold">
                    {item.description?.toUpperCase() ||
                      item.prodName?.toUpperCase()}
                  </td>
                  <td className="border-r-2 border-black px-1 uppercase font-mono text-[9px] leading-tight">
                    {item.huid || "-"}
                  </td>
                  ...
                  <td className="border-r-2 border-black px-1 font-mono">
                  ...
                  <td className="border-r-2 border-black px-1 font-mono">
                    {parseFloat(
                      item.basicAmt || (item.ntWt * item.rate) / 10 || 0,
                  ...
                  <td className="border-r-2 border-black px-1 font-mono">
                    {parseFloat(
                      item.finalMkg ||
                        (item.ntWt || 0) * (item.makingCharges || 0),
                    ).toFixed(1)}
                  </td>
                  <td className="border-r-2 border-black px-1">
                    {item.hallmarkCharges || item.hallmark || 0}
                  </td>
                  <td className="px-1 font-bold font-mono">
          ...
          <div className="w-full flex justify-between items-start">
            <div className="flex-1 space-y-6">
              <div>
                ...
                <ul className="text-[9px] list-none space-y-0.5">
                  ...
                  <li>
                    • When You Sell Gold The Spot Price Will Be Rupees 20 Less
                    Per Gram
                  </li>
                  <li>
                    • Goods Will Be Returned Only If The Purity Sepecified By Us
                    Is Not Found
                  </li>
              ...
              <div className="flex items-end gap-2 text-[10px] font-bold">
                <span className="whitespace-nowrap">
                  CASH PAY / Cash in Hand / CASH RECEIVED :
                </span>
                <span className="border-b border-black min-w-[120px] text-center pb-0.5">
                  ₹
                  {parseFloat(received).toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
            ...
            <div className="w-[320px] text-[11px] font-bold space-y-0.5 border-t border-black pt-1">
                ...
                <span>
                  {items
                    .reduce(
                      (acc, item) =>
                        acc +
                        parseFloat(item.amount || item.ntWt * item.rate || 0),
                      0,
                    )
              ...
              <div className="flex justify-between py-0.5 border-t border-black/10">
                ...
                <span>
                  {parseFloat(taxableAmt).toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              <div className="flex justify-between py-0.5">
                <span>CGST (1.5%) :</span>{" "}
                <span>
                  {parseFloat(cgst).toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
              <div className="flex justify-between py-0.5">
                <span>SGST (1.5%) :</span>{" "}
                <span>
                  {parseFloat(sgst).toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
