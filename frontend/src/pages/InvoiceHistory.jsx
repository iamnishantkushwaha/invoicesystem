...
const InvoiceHistory = () => {
  ...
  useEffect(() => {
    const filtered = invoices.filter((inv) => {
      const matchesFirm = selectedFirmId
        ? inv.firmId?._id === selectedFirmId
        : true;
      const matchesInvoiceType = selectedInvoiceTypeId
        ? inv.invoiceTypeId?._id === selectedInvoiceTypeId
        : true;
    ...
    <div className="page-container">
      <div className="content-max-width space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-6 border-b border-white/5">
          <div className="space-y-1 text-center md:text-left">
            ...
            <p className="text-theme-secondary text-sm">
              {selectedFirmId
                ? `Viewing invoices for ${firms.find((f) => f._id === selectedFirmId)?.name || "selected firm"}`
                : "Manage and review all your previously generated invoices"}
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 animate-fade-in relative z-30">
          <div
            className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end w-full ${location.state?.initialFirmId ? "lg:max-w-[560px] lg:grid-cols-2" : "lg:max-w-3xl"}`}
          >
            ...
            <div className="relative">
              ...
              {searchTerm && (
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-theme-muted hover:text-red-500 transition-colors"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 animate-pulse">
            ...
            <p className="text-theme-muted font-bold tracking-widest uppercase text-xs">
              Loading records...
            </p>
          </div>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-white/5 border-b border-white/10">
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest">
                      Customer
                    </th>
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest">
                      Invoice #
                    ...
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest text-center">
                      Type
                    </th>
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest">
                      Firm
                    </th>
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest text-right">
                      Amount
                    </th>
                    <th className="px-6 py-4 text-[10px] font-bold text-theme-muted uppercase tracking-widest text-center">
                      Actions
                  ...
                  {paginatedInvoices.map((inv, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        ...
                        <div className="text-[9px] text-theme-muted font-bold uppercase">
                          {inv.metalType}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                    ...
                    {[...Array(totalPages)].map((_, i) => {
                      ...
                      if (
                        ...
                        return (
                          <span
                            key={page}
                            className="text-theme-muted px-1 text-xs"
                          >
                            ...
                          </span>
                        );
